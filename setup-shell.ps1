Write-Host "➡️ Git: add customisations"
& git config --global include.path $pwd\git\.gitconfig

Write-Host "➡️ Powershell: get modules"
...
Write-Host  "➡️ Windows Terminal: Updating PWSH profile"
$runCmd = ". `"$pwd\\pwsh\\run.ps1`"";
If((Select-String -Path $PROFILE -Pattern $runCmd -SimpleMatch).Length -eq 0)
{
    Write-Host '➡️ Adding bootstrap script to $PROFILE'
